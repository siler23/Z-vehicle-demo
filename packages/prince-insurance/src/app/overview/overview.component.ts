...
export class OverviewComponent implements OnInit {
  ...
  constructor(private http: HttpClient) {
    ...
    let websocketURL = 'ws://myip:1880/ws/requestpolicy';
  ...
  get_number_cust_and_cars()
  {
    ...
    XMLReq.open("GET", "http://myip:3000/api/queries/Q1?insurer=resource%3Aorg.insurance.Insurer%23Prince");
  ...
  get_alerts(car_data:any) // VERY SLOW, CHANGE WHEN THERE ARE JOINS AVAILIBLE IN QUERIES FOR A QUERY OF UPDATE RECORDS THAT LINK TO CARS INSURED BY PRINCE
  {
    ...
    for(var i = 0; i < car_data.length; i++)
    {
      promises.push(new Promise(function(resolve, reject){
        ...
        var XMLReq = new XMLHttpRequest();
        XMLReq.open("GET", "http://myip:3000/api/queries/Q2?vehicleDetails=resource%3Aorg.vda.Vehicle%23"+vin);
        XMLReq.onreadystatechange = function() {
          if (XMLReq.readyState == XMLHttpRequest.DONE)
          {
